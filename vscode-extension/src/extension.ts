...
export function activate(context: vscode.ExtensionContext) {
  console.log(
    'Congratulations, your extension "deadrop" is now active!',
  );
  ...
  // const dropFileDisposable = vscode.commands.registerCommand(
  //   'deadrop.dropFile',
  //   async (event) => {
  ...
  const grabDisposable = vscode.commands.registerCommand(
    'deadrop.grab',
    () => {
      vscode.window.showInformationMessage(
        'Hello World from deadrop!',
      );
    },
  );
  vscode.window.showInformationMessage(
    'Hello World frogiuhm deadrop!',
  );
