import { DocsSectionTitle } from 'molecules/sections/SectionTitle';
import { CopyableCode } from 'atoms/Code';
import { INSTALL_COMMAND } from 'molecules/HeroBanner';

# ⌨️ deadrop CLI

The CLI gives you everything the web app does (dropping and grabbing) plus local and cloud-synced vaults for secret storage, a `.env` sync workflow, and `inject` for running scripts and CI/CD jobs with vault secrets in their environment, no file on disk.

<DocsSectionTitle
  id={'installation'}
  label={'📦 Installation'}
/>

### One-line installer (recommended)

Downloads the prebuilt binary for your platform into your home directory. No Node.js required.

<CopyableCode
  value={INSTALL_COMMAND}
  ariaLabel={'Copy install command'}
/>

If you are interested in the logic of the script, you can view it at `deadrop.io/install.sh`. It handles installing the appropriate binary for your platform and places it in your home directory for you.

### Via npm

If you'd rather manage it with your package manager:

```
npm install deadrop
```

Then invoke it through your package runner:

```
npx deadrop [command]
```

### Updating

Keep the CLI current with the built-in updater. It detects how you installed deadrop and updates in place, whether that's the prebuilt binary or the npm package:

```
deadrop update
```

### Requirements

For the npm install path: Node.js 20 or newer. The install.sh binary has no runtime dependencies.

<DocsSectionTitle
  id={'quick-start'}
  label={'🚀 Quick start'}
/>

Share a secret in one terminal:

```
deadrop drop "my-database-password"
```

Grab it in another (using the drop ID printed by the dropper):

```
deadrop grab <drop_id>
```

That's it. The two sessions complete a peer-to-peer handoff, the secret prints in the grabber's terminal, and both sessions exit.

<DocsSectionTitle
  id={'configuration'}
  label={'⚙️ Configuration'}
/>

Run the setup wizard once after install:

```
deadrop init
```

This creates a config file (default location is `.deadrop/` in your home directory) and your first local vault. The config tracks which vault is active and stores per-vault metadata:

<CopyableCode
  value={`active_vault:
  name: default
  environment: development
vaults:
  default:
    location: /<path-to-vault>/default.db
    environments:
      development: <base64-key>
      production: <base64-key>`}
  prompt={false}
  ariaLabel={'Copy config'}
/>

New vaults are seeded with `development` and `production` environments, each with its own encryption key for the local SQLite vault file. Treat those keys like any other secret. Never commit your `.deadrop/` directory.

<DocsSectionTitle
  id={'authentication'}
  label={'🔐 Authentication'}
/>

Authentication is only required for cloud-synced vaults and CI/CD service tokens. Drop and grab work without it.

```
deadrop login    # opens a browser to sign in via Clerk
deadrop logout
deadrop whoami   # check whether you're signed in
```

Tokens are stored in your OS keychain (macOS Keychain, Linux Secret Service via libsecret, Windows Credential Vault) and refreshed automatically — never written to disk in plaintext.

<DocsSectionTitle
  id={'commands'}
  label={'📤 Commands'}
/>

### `drop`

Share a secret with a single recipient over an end-to-end encrypted P2P connection.

<CopyableCode
  value={'deadrop drop [options] [input]'}
  prompt={false}
  ariaLabel={'Copy usage'}
/>

| Argument              | Description                                                  |
| --------------------- | ------------------------------------------------------------ |
| `[input]`             | The secret to drop. If omitted, you'll be prompted.          |
| `-i, --input <value>` | Pass the secret as a flag instead of a positional argument.  |
| `-f, --file`          | Treat the input as a file path and drop the file's contents. |

**Examples:**

```
# Drop a raw string
deadrop drop "ghp_xxxxxxxxxxxxxxxx"

# Drop the contents of a .env file
deadrop drop -f ./.env.production

# Drop a value piped from another command
echo "$DATABASE_URL" | deadrop drop
```

### `grab`

Connect to an active drop session using its ID.

```
deadrop grab <drop_id>
```

The grabbed secret is printed to stdout, so you can pipe or redirect:

```
deadrop grab abc123 > .env.local
```

### `vault`

Manage local and cloud-synced encrypted vaults.

#### `vault create <name> [location] [--cloud]`

Create a new vault. Without `--cloud`, the vault is stored locally as an encrypted SQLite file. With `--cloud`, a synced replica is created (requires authentication and a Supporter+ plan).

```
deadrop vault create work
deadrop vault create staging --cloud
deadrop vault create scratch ~/projects/scratch
```

#### `vault use <name> [--environment <env>]`

Switch the active vault. Subsequent `secret` and `drop` commands operate against this vault. Pass `--environment` to switch the active environment at the same time; without it the current environment is kept.

```
deadrop vault use staging
deadrop vault use staging -e production
```

#### `vault sync`

Sync the active vault to or from a `.env` file in the current directory. Useful for keeping a vault and a checked-in template in step during local development.

#### `vault export <name>`

Export the secrets in a vault to stdout in `.env` format.

```
deadrop vault export work > .env.work
```

#### `vault import <path>`

Read a `.env` file and add its keys and values to the active vault.

```
deadrop vault import ./.env.production
```

#### `vault delete <name>`

Remove a vault from config and delete its database file. This is destructive and cannot be undone.

#### `vault env`

Manage the environments in the active vault. Each environment has its own encryption key and its own set of secrets.

```
deadrop vault env list          # list environments (* marks the active one)
deadrop vault env add staging   # add a new environment with a fresh key
```

### `inject`

Run a command with a vault's secrets injected directly into its environment. Secrets are decrypted in memory and never written to disk, so there's no `.env` file to clean up.

<CopyableCode
  value={'deadrop inject [options] -- <command...>'}
  prompt={false}
  ariaLabel={'Copy usage'}
/>

| Option                    | Description                                                                                |
| ------------------------- | ------------------------------------------------------------------------------------------ |
| `-v, --vault <name>`      | Vault to inject (default: the active vault).                                               |
| `-e, --environment <env>` | Environment to inject (default: the active vault's environment).                           |
| `-c, --config <path>`     | Load an explicit config file (JSON or YAML) instead of the auto-discovered config, for CI. |
| `--no-override`           | Let pre-existing environment variables win over vault values (default: vault values win).  |
| `--refresh-token`         | Mint a fresh read-only Turso token before running, instead of reusing a cached one.        |
| `--verbose`               | Log the names of injected variables. Values are never logged.                              |

The `--` separator is required. Everything after it is passed straight through to your command, so its own flags are safe:

```
deadrop inject -- pnpm build
deadrop inject -v prod -e production -- node server.js
deadrop inject -- pnpm build --watch   # --watch reaches pnpm, not deadrop
```

The child process exits with the wrapped command's exit code, and Ctrl-C/SIGTERM are forwarded to it. For cloud vaults, `inject` mints a fresh read-only token on the fly whenever it doesn't already have a usable one, so `deadrop login` isn't strictly required.

**Config-free CI mode.** Set `DEADROP_VAULT_KEY` (the environment's decryption key) and `inject` skips config discovery entirely, so it works from a clean checkout with no `.deadrop/` directory. Select the vault and environment with `DEADROP_VAULT`/`DEADROP_ENVIRONMENT` (or `-v`/`-e`), and authenticate to the token endpoint with `DEADROP_API_KEY` when there's no interactive session:

```
DEADROP_API_KEY=... \
DEADROP_VAULT_KEY=... \
DEADROP_ENVIRONMENT=production \
deadrop inject -v prod -- pnpm build
```

### `desktop install`

Install (or update, if already installed) the [desktop app](/docs/overview#desktop-app). macOS only for now.

```
deadrop desktop install
deadrop desktop install --force   # reinstall even if already on the latest version
```

`deadrop update` also offers to update desktop automatically if it's already installed (pass `--skip-desktop` to opt out).

### `secret`

Manage secrets in the active vault.

#### `secret add [name] [value]`

Add a secret. If `name` or `value` is omitted, you'll be prompted.

```
deadrop secret add API_KEY "sk-..."
deadrop secret add    # interactive prompts for both
```

#### `secret remove [name]`

Remove a secret by name. Omit the name to pick interactively.

```
deadrop secret remove API_KEY
```

<DocsSectionTitle
  id={'patterns'}
  label={'💡 Patterns'}
/>

**Drop a file's contents safely.** Avoid copy-paste accidents by dropping the file directly:

```
deadrop drop -f ./.env.production
```

**Stream a vault into your shell.** Export to stdout and source it into your current process:

```
eval "$(deadrop vault export work)"
```

**Inline secrets in scripts without persisting them.** Grab a secret straight into an env var for the next command:

```
DB_URL=$(deadrop grab <drop_id>) ./deploy.sh
```

**Run CI/CD steps with vault secrets, no file on disk.** `inject` replaces the export-to-`.env`-then-source pattern. In CI, drive it entirely from environment variables, no config file or interactive login required:

```
DEADROP_API_KEY=... DEADROP_VAULT_KEY=... DEADROP_ENVIRONMENT=production \
  deadrop inject -v prod -- pnpm build
```

<DocsSectionTitle
  id={'related'}
  label={'🔗 Related'}
/>

- [VS Code extension](/docs/features/vscode) for the same drop, grab, and vault flows inside your editor
- [FAQs](/docs/faqs) for answers about security, accounts, and plans
- [Pricing](/pricing) for which features need a paid plan
