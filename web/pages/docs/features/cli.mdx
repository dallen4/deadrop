import { DocsSectionTitle } from 'molecules/sections/SectionTitle';
import { CopyableCode } from 'atoms/Code';
import { INSTALL_COMMAND } from 'molecules/HeroBanner';

# ⌨️ deadrop CLI

The CLI gives you everything the web app does (dropping and grabbing) plus local and cloud-synced vaults for secret storage, a `.env` sync workflow, and `inject` for running scripts and CI/CD jobs with vault secrets in their environment, no file on disk.

<DocsSectionTitle
  id={'installation'}
  label={'📦 Installation'}
/>

### One-line installer (recommended)

Downloads the prebuilt binary for your platform into your home directory. No Node.js required.

<CopyableCode
  value={INSTALL_COMMAND}
  ariaLabel={'Copy install command'}
/>

If you are interested in the logic of the script, you can view it at `deadrop.io/install.sh`. It handles installing the appropriate binary for your platform and places it in your home directory for you.

### Via npm

If you'd rather manage it with your package manager:

```
npm install deadrop
```

Then invoke it through your package runner:

```
npx deadrop [command]
```

### Updating

Keep the CLI current with the built-in updater. It detects how you installed deadrop and updates in place, whether that's the prebuilt binary or the npm package:

```
deadrop update
```

### Requirements

For the npm install path: Node.js 20 or newer. The install.sh binary has no runtime dependencies.

<DocsSectionTitle
  id={'quick-start'}
  label={'🚀 Quick start'}
/>

Share a secret in one terminal:

```
deadrop drop "my-database-password"
```

Grab it in another (using the drop ID printed by the dropper):

```
deadrop grab <drop_id>
```

That's it. The two sessions complete a peer-to-peer handoff, the secret prints in the grabber's terminal, and both sessions exit.

<DocsSectionTitle
  id={'configuration'}
  label={'⚙️ Configuration'}
/>

Run the setup wizard once after install:

```
deadrop init
```

This creates a `.deadroprc` config file in the current directory and your first local vault in a `.deadrop/` folder alongside it — config is scoped per project, so different projects can have different vaults. If a command runs somewhere with no project-scoped config, the CLI falls back to one shared default vault stored in your OS app-data directory — the same one the [desktop app](/docs/features/desktop) uses, so a vault created in either is immediately usable in the other with no import step. Pass `--global` to set that fallback up directly, or `-y` to accept the defaults without prompting. The config tracks which vault is active and stores per-vault metadata:

<CopyableCode
  value={`active_vault:
  name: default
  environment: development
vaults:
  default:
    location: /<path-to-vault>/default.db
    environments:
      development: <base64-key>
      production: <base64-key>`}
  prompt={false}
  ariaLabel={'Copy config'}
/>

New vaults are seeded with `development` and `production` environments, each with its own encryption key for the local SQLite vault file. Treat those keys like any other secret. Never commit your `.deadrop/` directory.

<DocsSectionTitle
  id={'authentication'}
  label={'🔐 Authentication'}
/>

Authentication is only required for cloud-synced vaults and CI/CD service tokens. Drop and grab work without it.

```
deadrop login    # opens a browser to sign in via Clerk
deadrop logout
deadrop whoami   # check whether you're signed in
```

Tokens are stored in your OS keychain (macOS Keychain, Linux Secret Service via libsecret, Windows Credential Vault) and refreshed automatically — never written to disk in plaintext.

<DocsSectionTitle
  id={'commands'}
  label={'📤 Commands'}
/>

### `drop`

Share a secret with a single recipient over an end-to-end encrypted P2P connection.

<CopyableCode
  value={'deadrop drop [options] [input]'}
  prompt={false}
  ariaLabel={'Copy usage'}
/>

| Argument              | Description                                                  |
| --------------------- | ------------------------------------------------------------ |
| `[input]`             | The secret to drop. If omitted, you'll be prompted.          |
| `-i, --input <value>` | Pass the secret as a flag instead of a positional argument.  |
| `-f, --file`          | Treat the input as a file path and drop the file's contents. |

**Examples:**

```
# Drop a raw string
deadrop drop "ghp_xxxxxxxxxxxxxxxx"

# Drop the contents of a .env file
deadrop drop -f ./.env.production

# Drop a value piped from another command
echo "$DATABASE_URL" | deadrop drop
```

### `grab`

Connect to an active drop session using its ID.

```
deadrop grab <drop_id>
```

The grabbed secret is printed to stdout, so you can pipe or redirect:

```
deadrop grab abc123 > .env.local
```

If the drop turns out to be a shared vault (see [`vault drop`](#vault) below), the CLI offers to add it to your config instead of printing it. It writes to the project-scoped `.deadroprc` if there is one, otherwise your global config, and asks which you want when neither exists.

### `vault`

Manage local and cloud-synced encrypted vaults.

#### `vault create <name> [location] [--cloud]`

Create a new vault. Without `--cloud`, the vault is stored locally as an encrypted SQLite file. With `--cloud`, a synced replica is created (requires authentication and a Supporter+ plan).

```
deadrop vault create work
deadrop vault create staging --cloud
deadrop vault create scratch ~/projects/scratch
```

#### `vault list`

List every vault in the config, marking the active one.

```
deadrop vault list
```

#### `vault use [name] [--environment <env>]`

Switch the active vault. Subsequent `secret` and `drop` commands operate against this vault. Pass `--environment` to switch the active environment at the same time; without it the current environment is kept. Omit the name and you'll be prompted to pick from your vaults.

```
deadrop vault use staging
deadrop vault use staging -e production
deadrop vault use              # pick from a list
```

#### `vault sync`

Sync the active vault to or from a `.env` file in the current directory. Useful for keeping a vault and a checked-in template in step during local development.

#### `vault export <name>`

Export the secrets in a vault to stdout in `.env` format.

```
deadrop vault export work > .env.work
```

#### `vault import <path>`

Read a `.env` file and add its keys and values to the active vault.

```
deadrop vault import ./.env.production
```

#### `vault drop [name]`

Share a cloud vault by dropping it. deadrop mints a read-only, expiring token for the environments you pick and hands it over a normal drop, so the recipient just runs `deadrop grab`. Cloud vaults only, since a local vault has nothing for them to connect to.

| Argument                | Description                                                     |
| ----------------------- | --------------------------------------------------------------- |
| `[name]`                | Vault to share. Defaults to the active vault.                   |
| `-e, --env <env...>`    | Environments to include. Defaults to the active environment.    |
| `--expires <duration>`  | How long the recipient's access lasts. Defaults to `30d`.       |
| `-g, --grabbers <n>`    | Number of recipients (requires a plan with multidrop).          |

```
deadrop vault drop
deadrop vault drop work -e production -e staging --expires 7d
```

You can only share a vault you own, since the token is minted against your account. To cut access before the token expires you have to rotate the vault's credentials, which kills every token including your own.

#### `vault delete <name>`

Remove a vault from config and delete its database file. This is destructive and cannot be undone.

#### `vault env`

Manage the environments in the active vault. Each environment has its own encryption key and its own set of secrets.

```
deadrop vault env list          # list environments (* marks the active one)
deadrop vault env add staging   # add a new environment with a fresh key
```

### `inject`

Run a command with a vault's secrets injected directly into its environment. Secrets are decrypted in memory and never written to disk, so there's no `.env` file to clean up.

<CopyableCode
  value={'deadrop inject [options] -- <command...>'}
  prompt={false}
  ariaLabel={'Copy usage'}
/>

| Option                    | Description                                                                                |
| ------------------------- | ------------------------------------------------------------------------------------------ |
| `-v, --vault <name>`      | Vault to inject (default: the active vault).                                               |
| `-e, --environment <env>` | Environment to inject (default: the active vault's environment).                           |
| `-c, --config <path>`     | Load an explicit config file (JSON or YAML) instead of the auto-discovered config, for CI. |
| `--no-override`           | Let pre-existing environment variables win over vault values (default: vault values win).  |
| `--refresh-token`         | Mint a fresh read-only Turso token before running, instead of reusing a cached one.        |
| `--ci`                    | Authenticate with a `DEADROP_API_KEY` instead of your signed-in session. See below.        |
| `--no-sync`               | Read a cloud vault directly instead of replicating it locally. Needed for read-only tokens. |
| `--only <names>`          | Inject only these secrets, comma-separated. Errors on a name the environment doesn't have, or one the API key doesn't permit. |
| `--prefix <prefix>`       | Prepend this to every injected variable name, e.g. `--prefix VITE_`.                        |
| `--verbose`               | Log the names of injected variables. Values are never logged.                              |

The `--` separator is required. Everything after it is passed straight through to your command, so its own flags are safe:

```
deadrop inject -- pnpm build
deadrop inject -v prod -e production -- node server.js
deadrop inject -- pnpm build --watch   # --watch reaches pnpm, not deadrop
```

The child process exits with the wrapped command's exit code, and Ctrl-C/SIGTERM are forwarded to it. For cloud vaults, `inject` mints a fresh read-only token on the fly whenever it doesn't already have a usable one, so `deadrop login` isn't strictly required.

**Config-free CI mode.** Set `DEADROP_VAULT_KEY` (the environment's decryption key) and `inject` skips config discovery entirely, so it works from a clean checkout with no `.deadrop/` directory. Select the vault and environment with `DEADROP_VAULT`/`DEADROP_ENVIRONMENT` (or `-v`/`-e`).

With a scoped API key from [`apiKeys create`](#apikeys) you don't need either. Pass `--ci` and the vault and environment both come from the key's own claims:

```
DEADROP_API_KEY=... \
DEADROP_VAULT_KEY=... \
deadrop inject --ci -- pnpm build
```

Those two variables are all a pipeline needs. `--ci` fails immediately naming whichever one is missing, rather than falling back to an interactive sign-in that can't succeed in a container. The token it mints is read-only and expires in five minutes.

Pair it with `--no-sync` for a cold read. Building a local replica is itself a write, which a read-only token forbids, so `--no-sync` reads the vault straight over the network instead — there's nothing to keep in sync when the replica is thrown away at the end of the job anyway. The same applies to a vault someone shared with you.

Secrets reach only the command `inject` spawns, so each step that needs them takes its own wrapper. Use `--only` and `--prefix` when one environment serves several jobs — `--only` matches the names as stored, and `--prefix` renames whatever survives the filter:

```
deadrop inject --ci --only API_URL,WEB_URL --prefix VITE_ -- pnpm build
```

That matters for bundlers: Vite inlines every `VITE_`-prefixed variable present at build time into the client bundle, referenced or not, so `--only` keeps an unrelated secret in the same environment out of what ships.

### `apiKeys`

Issue API keys for CI/CD, scoped to a single cloud vault and environment.

#### `apiKeys create`

Walks you through picking a cloud vault and one of its environments, then shows the key once. Use it as `DEADROP_API_KEY` with [`inject --ci`](#inject).

```
deadrop apiKeys create
deadrop apiKeys create -v my-app -e production
```

| Option                    | Description                                                          |
| ------------------------- | -------------------------------------------------------------------- |
| `-v, --vault <name>`      | Cloud vault to scope the key to (prompts to select when omitted).    |
| `-e, --environment <env>` | Environment to scope the key to (prompts to select when omitted).    |
| `--only <names>`          | Comma-separated secrets this key injects. Defaults to the whole environment. |
| `--prefix <prefix>`       | Prepend this to every variable name the key injects.                 |
| `-y, --yes`               | Skip the confirmation prompt.                                        |
| `--copy`                  | Copy the key to your clipboard without displaying it.                |
| `--print`                 | Write the key to stdout so it can be piped.                          |

`--only` and `--prefix` travel with the key, so a pipeline sets `DEADROP_API_KEY` and gets the right variables under the right names without repeating the flags at every call site. A run can narrow what the key allows but never widen it: [`inject --only`](#inject) may pick a subset of the key's list, and naming anything outside it fails. An `inject --prefix` that disagrees with the key's warns and the key's prefix is used.

Both shape what `inject` writes into the process; neither limits what the key can read. Secrets never pass through our servers, so the filtering happens on your machine. What separates environments is that each has its own encryption key, so a key issued for `production` cannot decrypt anything from `development`. Secret and environment names are stored unencrypted and stay visible across environments, the same way a committed `.env.example` lists names without values.

Only cloud vaults are offered — a key mints sync tokens for a remote database, so a local-only vault has nothing for it to authorize.

The key is shown on a separate screen that leaves nothing in your terminal scrollback once you dismiss it, and it can't be retrieved afterwards. Writing one to a non-interactive stream requires `--print`, so a script can't capture one by accident. List and revoke your keys from your Clerk account settings; each is named with its vault, environment, and issue time.

### `desktop install`

Install (or update, if already installed) the [desktop app](/docs/features/desktop). Works on macOS, Windows, and Linux, installing silently on all three (macOS via `.dmg`, Windows via a silent NSIS install, Linux via an AppImage placed in `~/.local/bin`).

```
deadrop desktop install
deadrop desktop install --force   # reinstall even if already on the latest version
```

`deadrop update` also offers to update desktop automatically if it's already installed (pass `--skip-desktop` to opt out).

### `desktop uninstall`

Remove the desktop app. On Linux this also removes the `.desktop` entry and the icon that the install registered, so no dead launcher is left behind in the application menu. On macOS it removes `/Applications/deadrop.app`. On Windows it points you at Settings > Apps, since the NSIS installer owns its own uninstaller.

```
deadrop desktop uninstall
```

This manages the AppImage install only. If you installed a `.deb` or `.rpm`, remove it with your package manager instead.

### `secret`

Manage secrets in the active vault.

#### `secret add [name] [value]`

Add a secret. If `name` or `value` is omitted, you'll be prompted.

```
deadrop secret add API_KEY "sk-..."
deadrop secret add    # interactive prompts for both
```

#### `secret remove [name]`

Remove a secret by name. Omit the name to pick interactively.

```
deadrop secret remove API_KEY
```

<DocsSectionTitle
  id={'patterns'}
  label={'💡 Patterns'}
/>

**Drop a file's contents safely.** Avoid copy-paste accidents by dropping the file directly:

```
deadrop drop -f ./.env.production
```

**Stream a vault into your shell.** Export to stdout and source it into your current process:

```
eval "$(deadrop vault export work)"
```

**Inline secrets in scripts without persisting them.** Grab a secret straight into an env var for the next command:

```
DB_URL=$(deadrop grab <drop_id>) ./deploy.sh
```

**Run CI/CD steps with vault secrets, no file on disk.** `inject` replaces the export-to-`.env`-then-source pattern. In CI, drive it entirely from environment variables, no config file or interactive login required:

```
DEADROP_API_KEY=... DEADROP_VAULT_KEY=... DEADROP_ENVIRONMENT=production \
  deadrop inject -v prod -- pnpm build
```

<DocsSectionTitle
  id={'related'}
  label={'🔗 Related'}
/>

- [VS Code extension](/docs/features/vscode) for the same drop, grab, and vault flows inside your editor
- [FAQs](/docs/faqs) for answers about security, accounts, and plans
- [Pricing](/pricing) for which features need a paid plan
