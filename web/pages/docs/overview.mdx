...
<Image
  src={'/images/deadrop-flow-diagram.jpg'}
  alt={'deadrop core drop/grab flow diagram'}
  width={2294}
  height={1743}
  style={{
    width: '100%',
...
The deadrop VS Code extension is available on the Marketplace for Supporter+ plans. See the [VS Code docs](/docs/features/vscode) for details.

## Desktop App

An experimental native desktop app (macOS only for now) is available via `deadrop desktop install` from the CLI, or `install-desktop.sh`. It's built to be the "hub" for engaged users, with drop, grab, and a native vault in one window.
