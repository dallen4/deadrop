...
An experimental native desktop app (macOS only for now) is available via `deadrop desktop install` from the CLI, or `install-desktop.sh`. It's built to be the "hub" for engaged users — drop, grab, and a native vault in one window.
